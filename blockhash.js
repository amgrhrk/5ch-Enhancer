const BlockHash = {};
BlockHash.one_bits = [0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4];
BlockHash.hammingDistance = function (hash1, hash2) {
  var d = 0;
  var i;
...
BlockHash.median = function (data) {
...
BlockHash.translate_blocks_to_bits = function (blocks, pixels_per_block) {
  var half_block_value = pixels_per_block * 256 * 3 / 2;
  var bandsize = blocks.length / 4;
  for (var i = 0; i < 4; i++) {
    var m = BlockHash.median(blocks.slice(i * bandsize, (i + 1) * bandsize));
...
BlockHash.bits_to_hexhash = function (bitsArray) {
  var hex = [];
...
BlockHash.bmvbhash_even = function (data, bits) {
...
BlockHash.bmvbhash = function (data, bits) {
  var result = [];
  var i, j, x, y;
  ...
  BlockHash.translate_blocks_to_bits(result, block_width * block_height);
  return BlockHash.bits_to_hexhash(result);
};
BlockHash.blockhashData = function (imgData, bits, method) {
  var hash;
  if (method === 1) {
    hash = BlockHash.bmvbhash_even(imgData, bits);
  }
  else if (method === 2) {
    hash = BlockHash.bmvbhash(imgData, bits);
  }
  else {
...
BlockHash.blockhash = function (arrayBuffer, bits, method, callback) {
  var imgData, hash;
  try {
    imgData = self['jpeg-js'].decode(arrayBuffer, { useTArray: true });
    if (!imgData) {
...
self['jpeg-js'].decode = function (jpegData, userOpts = {}) {
  var defaultOpts = {
...
self.addEventListener('message', function (e) {
	BlockHash.blockhash(e.data.img, 16, 2, (err, hash) => {
    self.postMessage({ id: e.data.id, err: err, hash: hash })
  })
})